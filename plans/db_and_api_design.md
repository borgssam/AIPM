...
*   **버튼 및 모달 구성:** 칸반 탭 상단 "새 할 일 추가" 버튼 오른쪽에 🤖 아이콘이 포함된 그라디언트 형태의 "인공지능 플링크" 버튼을 구현합니다. 이 버튼을 클릭 시 할 일 추천 모달(`isAiFlinkModalOpen`)이 활성화됩니다.
*   **모달 레이아웃 구조:**
    1. **에픽 선택 드롭다운:** 프로젝트에 포함된 에픽(`epics`) 중 하나를 고르는 콤보박스.
    2. **필터 체크박스:** "기존할일제외" 체크박스 (기존 등록된 할 일들을 필터링하기 위한 옵션 UI).
    3. **추천받기 버튼:** 추천 프로세스를 트리거하기 위한 버튼 UI.
    4. **추천 할 일 그리드:** 추천된 할 일 제목, 설명 등을 표기하는 테이블/그리드 뷰. 각 행 좌측에 체크박스가 포함됩니다.
    5. **하단 액션 영역:** 선택한 할 일들을 확정 반영하는 "적용" 버튼과 모달을 닫는 "닫기" 버튼 배치.
*   **동작 제약:** 이번 단계에서는 오직 화면 UI(모달 레이아웃 및 상태 노출)만 생성하고 실제 추천 로직이나 백엔드 연계 등의 추가 로직은 수행하지 않습니다.
